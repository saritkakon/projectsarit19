...
public class Camera {
	...
	/**
	 * The function gets a list of rays, and each time takes the ray and adds to it
	 * all the rays coming on the x, y, z axis and adds to each ray a random number
	 * between radius and -radius and adds it to the list of rays and creates a cone
	 * of all rays and returns the cone
	 * 
	 * @param ray
	 * @return
	 */
	public static List<Ray> constructRayCone(Ray ray) {
		...
		for (int i = 0; i < NUBER_OF_RAYS; i++) {
			double x = ray.getDir().getHead().getX() + ThreadLocalRandom.current().nextDouble(-RADIUS, RADIUS);
			double y = ray.getDir().getHead().getY() + ThreadLocalRandom.current().nextDouble(-RADIUS, RADIUS);

			double z = ray.getDir().getHead().getZ() + ThreadLocalRandom.current().nextDouble(-RADIUS, RADIUS);
